...
gaussian_module = Extension(
    "flip_gaussian",          # Name of the module
    sources=["py_gaussian.c", "gaussian.c", "bbox.c", "ziggurat_inline.c"],  # Source file
    include_dirs=[np.get_include()],
    extra_compile_args=["-O3", "-march=native", "-flto", "-fstrict-aliasing", "-ffast-math", "-funroll-loops", "-I" + np.get_include()],  # Enable profiling and include numpy headers
    extra_link_args=["-O3", "-march=native", "-flto", "-fopenmp"]  # Link with profiling support
)
...
setup(
    name="flip_gaussian",
    version="1.0",
    description="C extension for sampling from a 2D Gaussian",
